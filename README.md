...
### 인사이트
- AI 챗 인사이트 — 상단 총자산 그래프가 대화에 따라 전환(산다vs참는다 시뮬 · 저축 투영 · 메이트 비교)
- 오늘의 총평 자동 발화(마이 탭과 동일 셀렉터 수치) + 자세히 펼침
- 위젯: 월 저축액 슬라이더(파리 도달 시점 실시간) · O/X 퀴즈(+60P) · 추천 미션(미션 탭 연계)
- 키워드 매칭 스크립트 챗(+ 추천 칩 템플릿 삽입), 대화 저장·햄버거 다시보기
