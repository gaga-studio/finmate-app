...
처음에는 백엔드 없이 전 데이터가 시드 기반 목(mock)이었고, **시연 영상 촬영 재현성**
(고정된 오늘 날짜·고정 시드)을 최우선으로 설계했다.
...
| 서버 모드 | `VITE_API_URL` 설정 | [finmate-api](https://github.com/gaga-studio/finmate-api)의 실제 원장 |
...
목 모드를 남긴 이유는 촬영 재현성이다. 그 요구사항은 사라지지 않았다.
어느 쪽인지는 `src/data/source.tsx` 한 곳에서 정하고, 컴포넌트는 알 필요가 없다.
...
### 피드
- 그룹 찾기(소득/소비/지역·팔로잉 필터), 오늘의 스토리 12장 그리드 → layoutId 확대·좋아요·프로필 시트

### 미션
- 내 포인트(골드) + 포인트 상점 바텀시트
- "예산을 지켜라" 챌린지 — 일/주/월 판정 전부 거래 파생(일 예산·월 예산 ¼·수입 대비), 오늘 도트 탭 체크
- 진행 미션(파생 프로그레스) · 추천 미션(해체분석 근거 + 담기)

### 다이어리
- 인스타형 최신순 갤러리(3열) — 오늘만 그림·탭 가능, 과거는 일별 수입/지출 타일
- 오늘 탭 → 자동 기록 카드 3장(소비/자산/투자) 가로 스와이프

### 인사이트 (Cleo식 AI 챗)
- 상단 고정 그래프 = **습관 기반 미래 6개월 투영**(7~12월, 이번 달 실측 저축+투자 흐름) — 대화에 따라 산다vs참는다 시뮬·저축 투영으로 전환
- 그래프 우상단 **비교** 버튼 → 메이트/그룹을 골라 내 투영 위에 선 겹치기(재탭 해제)
- 오늘의 총평 자동 발화 + AI 응답 위젯: 추천옵션(즉답 버튼) · 월 저축액 슬라이더 · O/X 퀴즈(+60P) · 추천 미션(미션 탭 연계) · **리포트 생성**(7월 한 장 리포트 오버레이)
- 키워드 매칭 스크립트 챗, 대화 저장·햄버거 서랍 다시보기
...
- [x] M5a — 미션 탭: 포인트 / 챌린지 스트릭(거래 파생 판정) / 미션 / 추천 / 상점
- [x] M5b — 인사이트 탭: AI 챗(시뮬·슬라이더·메이트 비교·퀴즈·미션) + 대화 저장
- [x] M6 — 피드 + 다이어리: 스토리 그리드 / 프로필 시트 / 그림일기 갤러리
